...
{
    QSettings settings("University of Georgia", "LiveFit");
    ...
    settings.setValue("ballXYCovariance", ui.xYCovarianceDoubleSpinBox->value());
    settings.endGroup();
...
void LiveFitWindow::readSettings()
{
    ...
    ui.blurSizeSpin->setValue(
                settings.value("ballBlobBlur", 3.0).toDouble());
    ui.threshValDoubleSpinBox->setValue(
                settings.value("ballBlobThresh", 4.0).toDouble());
    ui.minRadiusDoubleSpinBox->setValue(
                settings.value("ballBlobMinRadius", 2.0).toDouble());
    ui.maxRadiusDoubleSpinBox->setValue(
                settings.value("ballBlobMaxRadius", 20.0).toDouble());
    ui.gravitySpin->setValue(
                settings.value("ballGravConstant", 470.0).toDouble());
    ui.frameComboBox->setCurrentIndex(
                settings.value("frameDisplayType", 0).toInt());
    ui.xYCovarianceDoubleSpinBox->setValue(
                settings.value("ballXYCovariance", 1.0).toDouble());
    settings.endGroup();
